...
# A script to send authenticated requests to the function node.
#
# Usage:
# ./send_request.sh <endpoint> <request_body>
#
# Example:
# ./send_request.sh "/v1/chat/completions" '{"model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "Hello!"}]}'
#
...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <endpoint> <request_body>"
    exit 1
fi
